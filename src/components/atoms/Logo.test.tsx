...
describe('Logo', () => {
  ...
  it('applies the correct size classes to the image', () => {
    ...
    expect(screen.getByAltText('Pair Up Events logo')).toHaveClass('h-3 w-3');

    rerender(<Logo size="sm" />);
    expect(screen.getByAltText('Pair Up Events logo')).toHaveClass('h-4 w-4');

    rerender(<Logo size="md" />);
    expect(screen.getByAltText('Pair Up Events logo')).toHaveClass('h-6 w-6');

    rerender(<Logo size="lg" />);
    expect(screen.getByAltText('Pair Up Events logo')).toHaveClass('h-8 w-8');

    rerender(<Logo size="xl" />);
    expect(screen.getByAltText('Pair Up Events logo')).toHaveClass('h-12 w-12');

    rerender(<Logo size="xxl" />);
    expect(screen.getByAltText('Pair Up Events logo')).toHaveClass('h-16 w-16');

    rerender(<Logo size="xxxl" />);
    expect(screen.getByAltText('Pair Up Events logo')).toHaveClass('h-24 w-24');

    rerender(<Logo size="hero" />);
    expect(screen.getByAltText('Pair Up Events logo')).toHaveClass('h-64 w-64 md:h-80 md:w-80');
  ...
  it('applies the correct text size classes', () => {
    ...
    expect(screen.getByText('Pair').parentElement).toHaveClass('text-4xl md:text-5xl lg:text-6xl');
